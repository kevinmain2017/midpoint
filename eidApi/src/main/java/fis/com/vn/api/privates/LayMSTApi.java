...
@RestController
public class LayMSTApi extends BaseApi{
	@Autowired MUserRepository mUserRepository;
	
	@GetMapping(value = "/private/lay-thong-tin-mst", produces = MediaType.APPLICATION_JSON_VALUE)
	public String mst(HttpServletRequest req, Authentication authentication) {
		...
		try {
			...
			mUserGet.setMstCaNhan(mUser.getMstCaNhan());
			mUserGet.setDiaChi(mUser.getDiaChi());
